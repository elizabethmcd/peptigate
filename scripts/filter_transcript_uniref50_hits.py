...
def filter_blast_results(input_file, output_blast, output_names):
    ...
    This function filters out BLAST hits with an e-value less than 1e-10 and a subject (hit)
    sequence length (slen) of less than 100 amino acids.
    These thresholds are chosen to select transcripts likely hitting long protein coding sequences
    in databases like UniRef50, thus suggesting that these transcripts are less likely to encode
    short open reading frame (sORF) peptides because they are likely fragments of true long
    ...
    filtered_blast_hits = blast_hits[(blast_hits["evalue"] < 1e-10) & (blast_hits["slen"] > 100)]
