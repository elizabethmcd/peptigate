    ...
    with open(fasta_file, "r") as f:
        ...
        for line in f:
            ...
            if line.startswith(">"):
                ...
                sequence_id = line[1:].split()[
                    0
                ]  # Assume ID is the first part after '>'
                sequence = ""
            else:
                sequence += line
        if sequence_id:
...
def extract_peptide_sequences(
    data, fasta_file, genes_output_file, peptides_output_file
):
    """Extract gene and peptide sequences based on the data dictionary and FASTA file, then write to separate files."""
    sequences = read_fasta(fasta_file)

    with open(genes_output_file, "w") as genes_out, open(
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print(
            "Usage: python extract_deeppeptide_sequences.py <json_file> <fasta_file> <genes_output_file> <peptides_output_file>"
        )
        sys.exit(1)
