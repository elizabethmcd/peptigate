...
metadata <- read_tsv(snakemake@input[['metadata']]) %>%
  select(organism, genome = genome_abbreviation)
...
coding_validation_filtered <- all_fai_filtered %>%
  ...
  filter(!sequence %in% train$sequence_id)             # remove sequences that overlap with the training data
...
write.table(coding_validation_filtered$sequence, snakemake_output[2], quote = F, col.names = F, row.names = F)
write.table(noncoding_train$sequence, snakemake_output[3], quote = F, col.names = F, row.names = F)
write.table(noncoding_validation_filtered$sequence, snakemake_output[4], quote = F, col.names = F, row.names = F)
