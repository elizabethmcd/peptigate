...
rule filter_input_to_proteins_shorter_than_100aa:
    """
    Here we filter the input protein sequences by length.
    If they are less than or equal to 100 amino acids, we demarcate them as potential sORFs.
    """
    input:
        ORFS_AMINO_ACIDS,
    output:
        peptide_faa=OUTPUT_DIR / "sORF" / "sorf_peptides.faa",
    conda:
        "envs/seqkit.yml"
    shell:
        """
...
rule convert_sorf_peptide_faa_to_tsv:
    input:
        peptide_faa=rules.filter_input_to_proteins_shorter_than_100aa.output.peptide_faa,
    output:
        tsv=OUTPUT_DIR / "sORF" / "sorf_peptides.tsv",
    conda:
        "envs/seqkit.yml"
    shell:
        """
...
rule combine_peptide_faa_predictions:
    input:
        sorf=rules.filter_input_to_proteins_shorter_than_100aa.output.peptide_faa,
